...
line = new File("../inputs/P_059.txt").text
lines = line.split(',').collect{Integer.parseInt(it)}
lines.each {
  //println it
}
println "Size: ${lines.size()}"

combs = Collections.nCopies(3, 'a'..'z').combinations()
println "combs: ${combs.size()}"
combs.take(15).each {println it}


println "j: ${((int)'z'-(int)'a'+1)**3}"